...
# Create indexes function - NON-BLOCKING
def create_indexes():
    try:
        database_url = os.getenv("DATABASE_URL")
        if not database_url:
            print("DATABASE_URL not found - skipping indexes")
            return
        ...
        print(f"Index creation failed (non-critical): {e}")

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Startup: Run index creation in background
    thread = threading.Thread(target=create_indexes)
    thread.daemon = True
    thread.start()
    yield
